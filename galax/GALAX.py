...
def search_bw_lw_ISA(X, y, coords, bw_min=None, bw_max=None, step=1, kernel='bisquare', task='regression', min_samples_per_class=5):
    ...
    bw_min : int, optional
        Minimum bandwidth to test. If None, defaults to max(5% of samples, n_features + 2, 20).
    bw_max : int, optional
        Maximum bandwidth to test. If None, defaults to max(95% of samples, n_features + 2).
    step : int, default=1
    ...
    min_samples_per_class : int, default=5
        For classification tasks, minimum required samples per class at each location.
...
class GALAX:
    ...
    automl_settings : dict, optional
        Settings for FLAML AutoML. If None, uses default settings.
